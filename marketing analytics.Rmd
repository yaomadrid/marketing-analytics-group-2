...
```{r}
setwd("~/Desktop/2018 spring/market analytics")
install.packages("reader")
library(readr)
ny_hmda <- read_csv("ny_hmda_2015.csv")
library(dplyr)
library(ggplot2)
```
...
sd(ny_hmda$loan_amount_000s, na.rm = TRUE)
...
For those variables with less than half of observations missing, we firstly test their independence with our dependent variable action_taken_name. As action_taken_name is a categorical variable, we could test its indepence with categorical variables using chi2 test and use t-test to test its indenpendence with continous variables. We found p-values in all test are less than 0.05, which means those variables are not independent with action_taken_name. 
...
For msamd, as there are 36375 (about 10% of the total observations) missing value in this variable, imputation might lead to inaccuray, we replaced the missing value with 0 and created a new dummy variable
...
For demographic characteristics like sex, race and ethinity, the missing data is correlated with action_taken_name. Therefore, we cannot delete the missing obervations directly. Instead, we use mode value to replace the missing value and add one dummy variable indicating the value is missing.
...
#or we could use following code to do the t.test
t.test(ny_hmApproval$Loan_Approval,ny_hmApproval$loan_amount_000s)
...
#or we can divide observations in to low-income and high-income
chisq.test(table(ny_hmApproval$Loan_Approval,ny_hmApproval$applicant_income_000s))
...
#Q8
```{r}
loan_approval_model1<-glm(Loan_Approval~loan_amount_000s+hud_median_family_income+factor(applicant_sex)+factor(applicant_race_1), family = binomial(link = 'probit'),data=ny_hmApproval)
summary(loan_approval_model1)
```
#Q9
```{r}
...
train_nyhm <- ny_hmApproval[train.index, ]
test_nyhm <- ny_hmApproval[-train.index, ]

loan_approval_model2<-lm(Loan_Approval~loan_amount_000s+hud_median_family_income+applicant_income_000s+applicant_sex+factor(applicant_race_1)+factor(preapproval)+factor(lien_status)+factor(loan_purpose)+factor(loan_type), data=train_nyhm)
summary(loan_approval_model2)

loan_approval_model3<-lm(Loan_Approval~loan_amount_000s+hud_median_family_income+applicant_income_000s+applicant_sex+factor(applicant_race_1)+factor(preapproval)+factor(lien_status)+factor(loan_purpose)+factor(loan_type), data=train_nyhm)
summary(loan_approval_model2)
