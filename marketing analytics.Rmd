...
library(pastecs)
stat.desc(RatioVariable, desc=TRUE)
options(scipen=100)
options(digits=1)
...
7. Define a variable Loan_Approval
```{r }
library(dplyr)
...
```
In our model, "Loan_Approval"" is a dependent variabl. It is a dummy variable and shows whether the application is approved or not. "loan_amount_000s", "hud_median_family_income","applicant_sex", "applicant_race_1", "IsMissingSex","IsMissingRace" are included in the model as dependent variables. 
...
print(summary(loan_approval_model3),digits=2)
Loan_Approval_Output <- predict(loan_approval_model2,newdata = test_data)

Comparing <- data.frame(actual=test_data$action_taken_name,Loan_Approval_Output)
head(Comparing)
...
#10#
In our model, there are significant customer segments based on demographics.
